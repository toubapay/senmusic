...
```bash
gcloud storage buckets create gs://promusic-originals --location=europe-west1
gcloud storage buckets create gs://promusic-hls --location=europe-west1
```
...
gcloud run deploy hls-transcoder \
  ...
  --concurrency 1 \
  --max-instances 10 \
  --set-env-vars ORIGINALS_BUCKET=promusic-originals,HLS_BUCKET=promusic-hls \
...
gcloud eventarc triggers create on-track-upload \
  ...
  --event-filters="bucket=promusic-originals" \
...
gcloud storage cp test-song.wav gs://promusic-originals/originals/$TRACK_ID/test-song.wav

# watch the logs
gcloud run services logs tail hls-transcoder --region europe-west1
```

Expected result in `gs://promusic-hls/hls/{trackId}/`:
